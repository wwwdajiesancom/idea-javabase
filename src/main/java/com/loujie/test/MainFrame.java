...
public class MainFrame extends JFrame {
    ...
    public MainFrame() {
        ...
        setLocation(
                (int) screenSize.getWidth() - WIDTH,
                (int) (screenSize.getHeight()
                        - tk.getScreenInsets(getGraphicsConfiguration()).bottom - HEIGHT));
    ...
    class PlayerFrame extends JFrame {
        ...
        public PlayerFrame() {
            ...
            setUndecorated(true);
            setBounds((int) (screenSize.getWidth() / 8),
                    (int) (screenSize.getHeight() / 8),
                    (int) (screenSize.getWidth() / 4 * 3),
                    (int) (screenSize.getHeight() / 4 * 3));
                ...
                protected void paintComponent(Graphics g) {
                    ...
                    if (null != lastImage) {
                        ...
                        if (w * getHeight() > h * getWidth()) {
                            g.drawImage(lastImage, 0,
                                    (int) (getHeight() - getWidth() * 1.0 / w
                                            * h) / 2, getWidth(),
                                    (int) (getWidth() * 1.0 / w * h), null);
                        } else {
                            g.drawImage(lastImage,
                                    (int) (getWidth() - getHeight() * 1.0 / h
                                            * w) / 2, 0, (int) (getHeight()
                                            * 1.0 / h * w), getHeight(), null);
                        }
                    }
                    ...
                    Point mouseLocation = MouseInfo.getPointerInfo()
                            .getLocation();
                    if (new Rectangle(
                            PlayerFrame.this.getLocation().x,
                            PlayerFrame.this.getLocation().y + getHeight() - 70,
                            getWidth(), 50).contains(mouseLocation)) {
                        Color c = g.getColor();
                        g.setColor(new Color(0, 0, 100, 60));
                        g.fill3DRect(0, getHeight() - 70, getWidth(), 50, true);

                        g.setColor(new Color(97, 211, 225, 120));
                        g.fill3DRect(mouseLocation.x - PlayerFrame.this.getX()
                                - 1, getHeight() - 70, 3, 50, true);

                        g.fill3DRect(0, getHeight() - 70, mv.position()
                                * getWidth() / mv.length(), 50, true);
            ...
            canvas.addMouseListener(new MouseAdapter() {
                public void mouseClicked(MouseEvent e) {
                    if (new Rectangle(
                            PlayerFrame.this.getLocation().x,
                            PlayerFrame.this.getLocation().y + getHeight() - 70,
                            getWidth(), 50).contains(e.getLocationOnScreen()) && null != mv) {
                        mv.position((e.getLocationOnScreen().x - PlayerFrame.this
                                .getLocation().x) * mv.length() / getWidth());
                    }
                }
            });
...
class MyVideo implements Closeable {
    ...
    private static final String HEADER = "作者:xhz qq群:23278990";
    ...
    public synchronized void position(int newFrameIndex) {
        try {
            ...
            last = ImageIO.read(new ByteArrayInputStream(ZipHelper.unZip(
                    frame.getPicData()).toByteArray()));
            oldRgbData = ImageHelper.getRgb(last);
            for (int i = lastFullFrameIndex + 1; i <= frameIndex; i++) {
                frame = getFrame(i);
                int[] rgb = ImageHelper.byteArray2RgbArray(ZipHelper.unZip(
                        frame.getPicData()).toByteArray());
    ...
    public synchronized Object[] nextFrame() {
        try {
            ...
            byte[] audioData = ZipHelper.unZip(frame.getAudioData())
                    .toByteArray();
            int[] rgb = null;
            if (frame.getFlag() == Frame.FULL) {
                ...
                rgb = ImageHelper.byteArray2RgbArray(ZipHelper.unZip(
                        frame.getPicData()).toByteArray());
            }
    ...
    public void addFrame(Frame frame) {
        try {
            FrameIndex fi = new FrameIndex(frame.getFlag() == Frame.FULL,
                    frame.getAudioData().length, frame.getPicData().length,
                    (int) fileChannel.position());
            indexList.add(fi);
    ...
    public Frame getFrame(int index) {
        ...
        mbb.get(picData);
        mbb.get(audioData);
        return new Frame(frameIndex.isFull ? Frame.FULL : Frame.INCREMENTAL,
                picData, audioData);
    }
    ...
    static class FrameIndex {
        ...
        public FrameIndex(boolean isFull, int audioDataSize, int picDataSize,
                          int index) {
            this.isFull = isFull;
        ...
        public byte[] toByteArray() {
            ...
            System.arraycopy(ByteBuffer.allocate(4).putInt(audioDataSize)
                    .array(), 2, result, 1, 2);
            System.arraycopy(
                    ByteBuffer.allocate(4).putInt(picDataSize).array(), 1,
                    result, 3, 3);
            System.arraycopy(ByteBuffer.allocate(4).putInt(index).array(), 0,
                    result, 6, 4);
            return result;
        }
    ...
    static class Frame {
        public static final Frame TERMINATE_FRAME = new Frame((byte) 0, null,
                null);
...
class RecordHelper {
    ...
    private static final Rectangle rectangle = new Rectangle(Toolkit
            .getDefaultToolkit().getScreenSize());
    private static ScheduledExecutorService screenCapture;
    private static ScheduledExecutorService audioProcesser;
    private static final ExecutorService ioProcesser = Executors
            .newSingleThreadExecutor();
    private static final ExecutorService dataProcesser = Executors
            .newSingleThreadExecutor();
    ...
    public static void start(final String filePath) {
            ...
            public void run() {
                try {
                    ...
                    for (; ; ) {
                        ...
                        if (generateFull) {
                            latestRgb = ImageHelper
                                    .getRgb((BufferedImage) imageData[1]);
                            picData = ZipHelper.zip(
                                    ImageHelper.image2Stream(
                                            (BufferedImage) imageData[1])
                                            .toByteArray()).toByteArray();
                            flag = MyVideo.Frame.FULL;
                            lastFullPicTime = (Long) imageData[0];
                            frameQueue.put(new MyVideo.Frame(flag, picData,
                                    ZipHelper.zip(audioData).toByteArray()));
                            generateFull = false;
                            continue;
                        }

                        int[] temp = latestRgb.clone();
                        double rate = DEFAULT_FULL_PIC_RATE
                                - ((((Long) imageData[0]) - lastFullPicTime)
                                * DEFAULT_FULL_PIC_RATE / 1000 / FULL_PIC_PERIOD);
                        byte[] changeData = ImageHelper.getByteArrayChange(
                                temp, (BufferedImage) imageData[1], rate);
                        if (null != changeData) {
                            latestRgb = temp;
        ...
        ioProcesser.execute(new Runnable() {
            public void run() {
                MyVideo video = new MyVideo(new File(filePath), true,
                        RECORD_RATE);
                try {
                    for (; ; ) {
    ...
    public static void restart() {
        ...
        screenCapture.scheduleAtFixedRate(new Runnable() {
            public void run() {
                try {
                    screenQueue.put(new Object[]{System.currentTimeMillis(),
                            ImageHelper.getScreen(rectangle)});
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt();
                }
            }
...
final class ImageHelper {
    ...
    private static final int DEFAULT_RGB = (255 << 24) | (0 << 16) | (0 << 8)
            | (0 << 0);
     ...
     * @return
     */
    public static int[] getRgbChange(int[] oldRgbData,
     ...
     * @return
     */
    public static byte[] getByteArrayChange(int[] oldRgbData,
                                            BufferedImage currentImage, double allowPercent) {
        try {
            int h = currentImage.getHeight();
            int w = currentImage.getWidth();
            ...
            byte[] result = new byte[(chunkCountList.size() + 1) * 4
                    + rgbData.length];
            buffer.clear();
            int index = 0;
            System.arraycopy(buffer.putInt(chunkCountList.size()).array(), 0,
                    result, index, 4);
            for (int chunkSize : chunkCountList) {
                buffer.clear();
                System.arraycopy(buffer.putInt(chunkSize).array(), 0, result,
                        index += 4, 4);
            }
     ...
     * @param rgbChangeData
     */
    public static void setRgbArrayChange(BufferedImage lastImage,
                                         int[] oldRgbData, int[] rgbChangeData) {
        int h = lastImage.getHeight();
        int w = lastImage.getWidth();
    ...
    public static int[] byteArray2RgbArray(byte[] data) {
        ...
        for (int i = 0; i < chunkCount; i++) {
            buffer.clear();
            buffer.put((new byte[]{data[index + 0], data[index + 1],
...
final class AudioHelper {
    ...
    private static final AudioFormat DEFAULT_AUDIO_FORMAT = new AudioFormat(
            AudioFormat.Encoding.PCM_SIGNED, RATE, SAMPLE_SIZE_IN_BITS,
            CHANNELS, SAMPLE_SIZE_IN_BITS / 8 * CHANNELS, RATE, false);
